            ...
            <div
                ...
                <div class="flex-1 overflow-y-auto max-h-[250px] flex flex-col-reverse">
                    ...
                    <div v-for="(log, index) in combatLog" :key="index" class="mb-1">
                        ...
                        <span v-if="log.isIframeMiss" class="text-gray-600 italic">
...
const strike = () => {
    ...
    if (now - lastStrikeTime < (cooldownMs.value - 5)) return;
    ...
    if (!dpsInterval) {
        dpsInterval = setInterval(updateRealDPS, 100);
    }

    const timestampStr = new Date().toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' }) + '.' + Math.floor(now % 1000 / 100);
